...
class _AIRecommendationsScreenState extends State<AIRecommendationsScreen>
  ...
  Future<void> _loadRecommendations() async {
    ...
    try {
      ...
      // Load trending recommendations (using destination recommendations)
      ...
      final personalizedRecs = personalizedData.map((data) => 
        _mapToDestinationRecommendation(data)).toList();
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _getMatchColor(rec.matchScore.round()),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const Spacer(),
                      Text(
                        _getPriceRangeText(rec.priceRange),
                        style: AppTextStyles.bodySmall.copyWith(
                          color: AppColors.textSecondary,
                          fontWeight: FontWeight.w600,
                        ),
                  ...
                  if (rec.tags.isNotEmpty) ...[
                    Wrap(
                      spacing: 4,
                      runSpacing: 4,
                      children: rec.tags.take(3).map((tag) => Container(
                        padding: const EdgeInsets.symmetric(
                          horizontal: 8,
                          vertical: 4,
                        ),
  ...
  /// Convert AI service response to DestinationRecommendation
  DestinationRecommendation _mapToDestinationRecommendation(Map<String, dynamic> data) {
    ...
    return DestinationRecommendation(
      ...
      rating: (metadata['rating'] ?? 0.0).toDouble(),
      tags: List<String>.from(metadata['tags'] ?? []),
      priceRange: metadata['price_range'] ?? {},
      season: metadata['season'] ?? 'Year-round',
      matchScore: ((data['confidence'] ?? 0.0) * 100).toDouble(),
      reason: data['reasoning'] ?? 'Recommended based on your preferences',
    );
  }

  /// Get price range text from price range data
  String _getPriceRangeText(Map<String, dynamic> priceRange) {
    if (priceRange.isEmpty) return '\$\$';
    
    final level = priceRange['level'] ?? 2;
    if (level is int) {
